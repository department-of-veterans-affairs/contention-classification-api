...
def calculate_file_sha256(file_path: str, chunk_size: int = 4096) -> str:
    ...
    Args:
        ...
        chunk_size: Size of chunks to read from file (default: 4096)
...
def verify_file_sha256(file_path: str, expected_sha256: str, chunk_size: int = 4096) -> bool:
    ...
    try:
        ...
        logger.error(f"SHA-256 verification failed for {os.path.basename(file_path)}")
        ...
        logger.error(f"Error during SHA-256 verification for {file_path}: {e}")
...
def download_ml_models_from_s3(model_file: str, vectorizer_file: str, app_config: Dict[str, Any]) -> Tuple[str, str]:
    ...
    # Get chunk size from ML classifier config with default fallback
    chunk_size = 4096  # Default fallback
    if "hash_config" in verification_config:
        chunk_size = verification_config["hash_config"].get("chunk_size_bytes", 4096)
...
def _get_environment(app_config: Dict[str, Any]) -> str:
    ...
    default_env = app_config.get("s3_utilities", {}).get("default_environment", "staging")
    env = os.environ.get("ENV", default_env)
    if env not in app_config["aws"]["s3"]["buckets"]:
        logger.warning(f"Environment '{env}' not found in S3 bucket configuration, using '{default_env}'")
        env = default_env
    return env
...
def _get_expected_sha(verification_config: Dict[str, Any], file_type: str, env_var: str) -> str:
    ...
    if file_type not in verification_config["expected_checksums"]:
        return expected_sha

    config_value = verification_config["expected_checksums"][file_type]
    return str(config_value) if config_value is not None else expected_sha
