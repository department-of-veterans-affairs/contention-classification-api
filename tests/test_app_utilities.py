...
def test_download_models_from_s3_when_files_missing(
    mock_boto_client: MagicMock, mock_os_path: MagicMock, mock_verify_sha: MagicMock
) -> None:
    """Test that S3 download works correctly when model files are missing locally."""
    mock_s3_client = MagicMock()
    mock_boto_client.return_value = mock_s3_client

    # Mock os.path.exists to return False for initial file checks but True for SHA verification
    def mock_exists(path: str) -> bool:
        # Return True for model files to pass SHA verification
        if "models/" in path:
            return True
        return False
